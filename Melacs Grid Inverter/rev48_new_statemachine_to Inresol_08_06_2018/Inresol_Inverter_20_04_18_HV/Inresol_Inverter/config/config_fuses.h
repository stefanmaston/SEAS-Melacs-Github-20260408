/*
 *  File name: config_fuses.h
 *
 *  Description: Fuses settings
 *
...
// The fuses will select the oscillator source, power-up timers, watch-dog timers etc.
// The macros are defined within the device header files.
// The configuration fuse registers reside in Flash memory.

// DSPIC33EP64GS502 Configuration Bit Settings
...
#pragma config IOL1WAY = ON             // Peripheral pin select configuration bit (Allow only one reconfiguration)
#pragma config FCKSM = CSDCMD           // Clock Switching Mode bits (Both Clock switching and Fail-safe Clock Monitor are disabled)#pragma config PLLKEN = OFF             // PLL Lock Enable Bit (Clock switch will not wait for the PLL lock signal)
#endif   // USE_FRC_CLOCK
