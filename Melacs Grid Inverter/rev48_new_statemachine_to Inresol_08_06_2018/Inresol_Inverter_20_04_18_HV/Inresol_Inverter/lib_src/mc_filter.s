...
;=================== CODE =====================
          ...
          .section  .text
          .global   _MC_FilterUpdate_Assembly
          .global   MC_FilterUpdate_Assembly

_MC_FilterUpdate_Assembly:
MC_FilterUpdate_Assembly:
...
jPI5:
	 ...
	 ;; Return 1	  
	  ...
	  .global   _MC_Sum_Assembly
          .global   MC_Sum_Assembly
...
MC_Sum_Assembly:
     ;; Save CORCON, then configure as needed
          ...
          mov.w     CorconW4,CORCON
	 ;; workW0 : SUM_PARAMIN*
	  mov.w	    workW0,StateptrW1
	  mov.w     workW3,OutptrW2
	  mov.w	    [StateptrW1+SUM_PARMIN_S1_in],workW0
          lac       workW0,#0,A
	  mov.w	    [StateptrW1+SUM_PARMIN_S2_in],workW0
          lac       workW0,#0,B
          add       A
	  mov.w	    [StateptrW1+SUM_PARMIN_S3_in],workW7
	  mov.w	    [StateptrW1+SUM_PARMIN_omegaL_in],workW5
	  mpy       workW7*workW5,B
	  add	    A
          sac       A,#0,UnlimitW6
     ...
     ;;     Out = U 
          ...
          mov.w     [StateptrW1+SUM_PARMIN_out_Max],OutW3
          cp        UnlimitW6,OutW3
          bra       GT,jS5             ; U > Outmax; OutW3 = Outmax

          mov.w     [StateptrW1+SUM_PARMIN_out_Min],OutW3
          cp        UnlimitW6,OutW3
          bra       LE,jS5             ; U < Outmin; OutW3 = Outmin
