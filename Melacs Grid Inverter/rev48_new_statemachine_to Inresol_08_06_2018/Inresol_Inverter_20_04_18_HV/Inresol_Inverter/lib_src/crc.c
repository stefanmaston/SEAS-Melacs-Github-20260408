...
/* ========================================================================= */
uint32_t crc32(uint32_t crc, const uint8_t* buf, int16_t len)
{
	if(!buf) return 0L;
	crc = crc ^ 0xffffffffL;
	while (len >= 8)
...
void CRC32_Update( uint32_t *crcvalue, const uint8_t data )
{
	*crcvalue = crc_table[ ( *crcvalue ^ data ) & 0xff ] ^ ( *crcvalue >> 8 );
}

void CRC32_UpdateChecksum( uint32_t *crcvalue, const void *data, int16_t length )
{
	uint32_t crc;
...
uint32_t CRC32_BlockChecksum( const void *data, int16_t length )
{
	uint32_t crc;
