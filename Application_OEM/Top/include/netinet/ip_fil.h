...
/*	$NetBSD: ip_fil.h,v 1.20.2.2 1997/11/17 16:32:55 mrg Exp $	*/
...
extern	int	send_reset (struct tcpiphdr *);
//extern	void	ipfr_fastroute (mb_t *, fr_info_t *, frdest_t *);
#endif /* #ifndef _KERNEL */


extern	u_short	fr_tcpsum (mb_t *, ip_t *, tcphdr_t *);
#define	FR_SCANLIST(p, ip, fi, m)	fr_scanlist(p, ip, fi, m)
extern	int	fr_scanlist (int, ip_t *, fr_info_t *, void *);
extern	u_short	ipf_cksum (u_short *, int);
extern	int	fr_copytolog (int, char *, int);
extern	void	frflush (int, caddr_t);
extern	frgroup_t *fr_addgroup (u_short, frentry_t *, int, int);
extern	frgroup_t *fr_findgroup (u_short, u_32_t, int, int, frgroup_t ***);
extern	void	fr_delgroup (u_short, u_32_t, int, int);
